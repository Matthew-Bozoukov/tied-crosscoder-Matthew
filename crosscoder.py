...
class LossOutput(NamedTuple):
    ...
    l1_loss_shared: torch.Tensor
    l1_loss_exclusive: torch.Tensor
...
class CrossCoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.W_enc = nn.Parameter(
            torch.empty(2, d_in, 3, d_hidden, dtype=self.dtype)  
        )
        ...
        ).unsqueeze(2).repeat(1, 1, 3, 1)
        self.b_enc = nn.Parameter(torch.zeros(3, d_hidden, dtype=self.dtype))
    ...
    def encode(self, x, apply_relu=True):
        ...
        x_enc = einops.einsum(
            x,
            self.W_enc,
            "batch n_models d_model, n_models d_model three d_hidden -> batch three d_hidden",
        )
        if apply_relu:
    ...
    def decode(self, acts):
        # acts: [batch, d_hidden]
        acts_dec_shared = einops.einsum(
            acts[:, 0, :],
            self.W_dec,
            "batch d_hidden, d_hidden n_models d_model -> batch n_models d_model",
        )
        acts_dec_A = einops.einsum(
            acts[:, 1, :],
            self.W_dec[:, 0, :],
            "batch d_hidden, d_hidden d_model -> batch d_model",
        )
        acts_dec_B = einops.einsum(
            acts[:, 2, :],
            self.W_dec[:, 1, :],
            "batch d_hidden, d_hidden d_model -> batch d_model",
        )
        acts_dec_exclusive = torch.stack((acts_dec_A, acts_dec_B), dim=1)
        return acts_dec_shared + acts_dec_exclusive + self.b_dec
    ...
    def get_losses(self, x):
        ...
        l1_loss_shared = (acts[:, 0, :] * total_decoder_norm[None, :]).sum(-1).mean(0)
        l1_loss_exclusive = (acts[:, 1, :] * decoder_norms[None, :, 0] + acts[:, 2, :] * decoder_norms[None, :, 1]).sum(-1).mean(0)
        l0_loss = (acts>0).float().sum(-1).mean()

        return LossOutput(l2_loss=l2_loss, l1_loss_shared=l1_loss_shared, l1_loss_exclusive=l1_loss_exclusive, l0_loss=l0_loss, explained_variance=explained_variance, explained_variance_A=explained_variance_A, explained_variance_B=explained_variance_B)
